...
class Helpers {
    ...
    //remnant, not used anywhere at this point
    public static function getPortfolioData($db,$type,$portfolio_id,$currency_id,$from_month,$from_year,$to_month,$to_year,$options = [],&$error) {
        $error = '';
        $data = [];
        ...
        if($portfolios == 0) $error .= 'NO active portfolios found.';
        ...
        if($error === '') {
            ...
            foreach($portfolios as $pf_id=>$name) {
                ...
                $data[$pf_id]['name'] = $name;
                $data[$pf_id]['assets'] = self::getPortfolioAssets($db,$pf_id,$error_pf);
                if($error_pf === '') {
                    $data[$pf_id]['performance'] = self::performanceReport($db,$pf_id,$currency_id,$from_month,$from_year,$to_month,$to_year,$options_pf,$error_pf);
                    $data[$pf_id]['balances']  = self::assetBalancesReport($db,$pf_id,$currency_id,$from_month,$from_year,$to_month,$to_year,$options_pf,$error_pf);
                }    
            }
        }    
        ...
        if($portfolios == 0) {
            ...
            foreach($portfolios as $pf_id=>$name) {
                ...
                if($type === 'performance') {
                    ...
                    foreach($months as $month) {
                        $key = $month['year'].':'.$month['mon'];
                        if($error_pf === '') $return = $data[$key]['return']; else $return = 0;
                        $chart_data[$m][] = $return;
                        $m++;
                    }
    ...
    public static function performanceReportAll($db,$currency_id,$from_month,$from_year,$to_month,$to_year,$options = [],&$error)  {
        $error = '';
        $error_tmp = '';
        $html = '';
        $output = [];
        ...
        $sql = 'SELECT portfolio_id,name FROM '.TABLE_PREFIX.'portfolio  WHERE status = "OK"';
        $portfolios = $db->readSqlList($sql);
        if($portfolios == 0) $error .= 'No portfolios found with status = OK. ';
        ...
        $performance_all = [];
        //initialise each month with zero values
        foreach($months as $month) {
            $key = $month['year'].':'.$month['mon'];

            $performance_all[$key] = ['name' => $key,
        ...
        //sum all portfolio performance values 
        $options_portfolio = $options;             
        $options_portfolio['output'] = 'DATA';
        foreach($portfolios as $portfolio_id=>$name) {
            $error_pf = '';
            $performance = self::performanceReport($db,$portfolio_id,$currency_id,$from_month,$from_year,$to_month,$to_year,$options_portfolio,$error_pf);
            if($error_pf !== '') {
                $error .= 'Portfolio['.$name.'] error: '.$error_pf.'<br/>';
            } else {    
                foreach($months as $month) {
                    $key = $month['year'].':'.$month['mon'];

                    $performance_all[$key]['start_value'] += $performance[$key]['start_value'];
                    $performance_all[$key]['add'] += $performance[$key]['add'];
                    $performance_all[$key]['withdraw'] += $performance[$key]['withdraw'];
                    $performance_all[$key]['buy'] += $performance[$key]['buy'];
                    $performance_all[$key]['sell'] += $performance[$key]['sell'];
                    $performance_all[$key]['income'] += $performance[$key]['income'];
                    $performance_all[$key]['expense'] += $performance[$key]['expense'];
                    $performance_all[$key]['end_value'] += $performance[$key]['end_value'];
                }
            }    
        }

        //NB: Combined period returns, simple sum would be meaningless
        foreach($months as $month) {
            $key = $month['year'].':'.$month['mon'];

            $period = $performance_all[$key];
            $performance_all[$key]['return'] = self::calcPeriodReturn($options['return_method'],$period);
        }
        ...
        if($error === '') {
            if($options['output'] === 'HTML') {
                ...
                $html = Html::arrayDumpHtml($performance_all,$html_options);
                return $html;
            }
    ...
    public static function assetBalancesReportAll($db,$currency_id,$from_month,$from_year,$to_month,$to_year,$options = [],&$error)  {
        $error = '';
        $error_tmp = '';
        $html = '';
        $output = [];
        ...
        //merge all portfolio monthly asset balances. NB: Assets are NOT portfolio specific so need to sum asset balnce where it occurs in more than one portfolio
        $options_portfolio = $options;             
        $options_portfolio['output'] = 'DATA';
        foreach($portfolios as $portfolio_id=>$name) {
            $error_pf = '';
            $balances = self::assetBalancesReport($db,$portfolio_id,$currency_id,$from_month,$from_year,$to_month,$to_year,$options_portfolio,$error_pf);
            if($error_pf !== '') {
                $error .= 'Portfolio['.$name.'] error: '.$error_pf.'<br/>';
            } else {    
                foreach($months as $month) {
                    ...
                    $period = $balances[$key];
                    foreach($period as $asset_id => $period_balance) {
                        //$period_balance['name'] = $name.': '.$period_balance['name'];
                        
                        if(isset($balances_all[$key][$asset_id])) {
                            $balances_all[$key][$asset_id]['start_units'] += $period_balance['start_units'];
                            $balances_all[$key][$asset_id]['start_value'] += $period_balance['start_value'];  
                            $balances_all[$key][$asset_id]['change_units'] += $period_balance['start_units']; 
                            $balances_all[$key][$asset_id]['end_units'] += $period_balance['end_units']; 
                            $balances_all[$key][$asset_id]['end_value'] += $period_balance['end_value']; 
                        } else {
                            $balances_all[$key][$asset_id] = $period_balance;  
                        }    
                    }
        ...
        if($error === '') {
            if($options['output'] === 'HTML') {
                ...
                foreach($months as $month) {
                    $key = $month['year'].':'.$month['mon'];
                    $html .= '<h1>'.$key.'</h1>'.Html::arrayDumpHtml($balances_all[$key],$html_options);
                }    
